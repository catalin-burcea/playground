 ...
 * This kid is an arbitrary alias for the key, and it's usually used by the Resource Server to select the correct entry
 * from the collection since the same key should be included in the JWT Header.
 * We face a new problem now; since Spring Security OAuth doesn't support JWK, the issued JWTs won't include the kid Header.
 *
 * For this reason, we need the following workaround.
 */
